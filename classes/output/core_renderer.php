...
class core_renderer extends \theme_boost\output\core_renderer {
    ...
    public function full_header() {
        ...
        if ($PAGE->context->contextlevel == CONTEXT_COURSE && $COURSE->id != SITEID && $USER->id) {
            $ufservice = \core_favourites\service_factory::get_service_for_user_context(\context_user::instance($USER->id));
            $header->isbookmarked = $ufservice->favourite_exists('core_course', 'courses', $COURSE->id,
                    \context_course::instance($COURSE->id));
            $header->canbookmark = true;
            $header->courseid = $COURSE->id;
        }

        $context = $PAGE->context;
        return $this->render_from_template('theme_nova/header', $header);
    }
    ...
    public function mega_menu($user = null) {
        ...
        return $this->render_from_template('theme_nova/megamenu', $template);
    }
    ...
    public function navbar() {
        ...
        foreach ($items as $item) {
            $item->hideicon = true;
            if ($item->hidden) {
                $item->text .= get_string('hiddennavitem', 'theme_nova');
            }
        ...
        return $this->render_from_template('theme_nova/breadcrumb', $template);
    }
    ...
    public function arup_logo() {
        if (preg_match('/page-local-onlineappraisal/', $this->page->bodyid)) {
            ...
            return $this->image_url('Arup_logo', 'theme_nova');
        }
    }

    public function appraisal_logo() {
        if (preg_match('/page-local-onlineappraisal/', $this->page->bodyid)) {
            return $this->image_url('appraisal', 'theme_nova');
        }
    }
    ...
    public function arup_pagecontent() {
        ...
        if ($layout == 'frontpage') {
            $theme = \theme_config::load('nova');
            $template = new \stdClass();
            $template->output = $this;
            $template->wwwroot = $CFG->wwwroot;
            $template->headertext = $this->page->theme->settings->frontpageheader;
            $template->headersub = $this->page->theme->settings->frontpagesub;
            ...
            return $this->render_from_template('theme_nova/'.$layout.'_content', $template);
        }
    }
    ...
    public function theme_header_image() {
        ...
        if ($image = $theme->setting_file_url('frontpageimage', 'frontpageimage')) {
            ...
            return $this->image_url('header/defaultbanner', 'theme_nova');
        }
    }
    ...
    protected function render_custom_menu(\custom_menu $menu) {
        ...
        foreach ($menu->get_children() as $item) {
            ...
            $custommenucontent .= $this->render_from_template('theme_nova/custom_menu_item', $context);
        }
    ...
    public function notifications() {
        ...
        if (empty($this->theme)) {
            $this->theme = \theme_config::load('nova');
        }
        ...
        if (isloggedin() && !isguestuser()) {
            if (empty($USER->timezone) || ($USER->timezone == 99)) {
                ...
                $text = get_string('pleasesettimezone', 'theme_nova', $settingurl->out());
                 $alerts[] = $this->notification($text, $type);
            }
        }
        ...
        if (count($alerts)) {
            ...
            return $this->render_from_template('theme_nova/notifications', $templateinfo);
        }
    }
    ...
    public function copyright() {
        return get_string('copyright', 'theme_nova', date('Y'));
    }
    ...
    public function footerlinks() {
        ...
        return $this->render_from_template('theme_nova/footer_links', $templateinfo);
    }

    /**
     * Check if the cohort footers are enabled
     * @return bool
     */
    public function hascohortfooters() {
        if (empty($this->theme)) {
            $this->theme = \theme_config::load('nova');
        }
        if (isset($this->theme->settings->enablecohortfooter) &&
            !empty($this->theme->settings->enablecohortfooter)) {
            return true;
        }
        return false;
    }
