...
use theme_nova\external\page_footer_exporter;
...
class external extends external_api {
    ...
    public static function get_footer_content_parameters() {
        return new external_function_parameters([
            ...
            'appraisal' => new external_value(PARAM_INT, 'Context ID', VALUE_DEFAULT, 0)
        ]);
    }
     ...
     * Get footer content.
     *
     * @param int $contextid Page Contect ID
     * @param int $appraisal 1 if in apprai
     *
     * @return  array list of courses and warnings
     */
    public static function get_footer_content($contextid, $appraisal) {
        global $PAGE, $DB, $USER;
        ...
        if (!empty($cohorts)) {
            ...
            $footer->content = format_text($first->description, $first->descriptionformat);
        ...
        // Get course based footer contacts.
        if ($context->contextlevel == CONTEXT_COURSE) {
            $roles = get_roles_with_capability('theme/nova:showinfooter', CAP_ALLOW, $context);
        ...
        foreach ($usercohorts as $uchort) {
            if (!empty($uchort->idnumber)) {
                $cohortshortname = $uchort->idnumber;
                $addon = $appraisal ? '_appraisal_footer' : '_footer';
                if ($footercohort = $DB->get_record('cohort', ['idnumber' => $cohortshortname . $addon])) {
                    $footer->content = format_text($footercohort->description, $footercohort->descriptionformat);
    ...
    public static function setcoverimage(\context $context, $type, $data, $filename, $cropped) {
        ...
        if (!in_array($ext, ['jpg', 'png', 'gif', 'svg', 'webp'])) {
            return ['success' => false, 'warning' => get_string('unsupportedcoverimagetype', 'theme_nova', $ext)];
        }
        ...
        if (strlen($binary) > get_max_upload_file_size($CFG->maxbytes)) {
            throw new \moodle_exception('error:coverimageexceedsmaxbytes', 'theme_nova');
        }
        ...
        if ($context->contextlevel === CONTEXT_COURSECAT && $type === 'catalogue') {
            ...
            if ($cropped) {
                $fs->delete_area_files($context->id, 'theme_nova', $filearea);
            } else {
                $fs->delete_area_files($context->id, 'theme_nova', $type);
                $fs->delete_area_files($context->id, 'theme_nova', $type. '_cropped');
            }
            ...
            if ($cropped) {
                ...
                // Make sure images are stored in the course overviewfiles too.
                ...
                $courseimage = array(
                    ...
                    'filepath' => '/',
                    'filename' => $newfilename);
                $fs->create_file_from_string($courseimage, $binary);
            }
        } else {
