...
function theme_nova_alter_css_urls_d(&$urls) {
    $urls = [];
    $theme = theme_config::load('nova');

    $plugincss = new moodle_url('/theme/nova/css.php');
    $plugincss->set_slashargument('/core/all.css');
    $urls[] = $plugincss;

    $themecss = new moodle_url('/theme/nova/css.php');
    $themecss->set_slashargument('/theme/default.css');
    $urls[] = $themecss;

    $fontcss = new moodle_url('/theme/nova/css.php');
 ...
 * @return string
 */
function theme_nova_get_main_scss_content() {
    global $CFG;
    return file_get_contents($CFG->dirroot . '/theme/nova/scss/default.scss');
}